...
$(document).ready(function load_key(e) {
    jQuery.support.cors = true;


    alert.window("block 0")
    ...
    //
    var defaultKey = "180fbadb-65ad-4ff6-b288-537219172b45";
    var profile = "car";
...
function setupRoutingAPI(map, ghRouting) {
    map.setView([-34.928589,138.59994289999997], 12);
            ...
            ghRouting.doRequest(function (json) {
                if (json.message) {
                    ...
                    routingLayer.addData({
                     ...
                     window.alert("Block 1");
                    var outHtml = "Distance in meter:" + path.distance;
                    outHtml += "<br/>Times in seconds:" + path.time / 1000;
                    var s = path.distance;
                    window.alert(s);
                    //This part calculates basic energy calculation
                    outHtml += "<br/>Total amount Energy KWH: " + (((16.67^2/(2*s)*13800)/1000)/3600);
...
function setupRouteOptimizationAPI(map, ghOptimization, ghRouting) {
    map.setView([-34.928589,138.59994289999997], 13);
    ...
    $("#dragable_buttons_1").click(function () {
        window.alert("loaded");
        $.getJSON("preloaded_gfts/route101.json?v=" + exampleVersion, function (jsonData) {

            clearMap();
            map.setView([-34.928589,138.59994289999997], 6);
            $("#vrp-response").text("Calculating ...");
            GraphHopperOptimization.gfts_test(jsonData, optimizeResponse);
             window.alert("loaded1");
        });
    });
    ...
    $("#set_example_us_tour").click(function () {
        $.getJSON("route-optimization-examples/american_road_trip.json?v=" + exampleVersion, function (jsonData) {
            ...
            clearMap();
            map.setView([-34.928589,138.59994289999997], 4);
            $("#vrp-response").text("Calculating ...");
            ghOptimization.doRequest(jsonData, optimizeResponse);
        });
    });
