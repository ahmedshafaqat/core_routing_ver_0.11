...
$(document).ready(function load_key(e) {
    ...
    var ghRouting = new GraphHopperRouting({key: defaultKey, host: host, vehicle: profile, elevation: true});
...
function setupRoutingAPI(map, ghRouting) {
    ...
    map.on('click', function (e) {
        ...
        //This part testing 
        if (ghRouting.points.length > 4 && ghRouting.points.length < 6) {
        ...
        if (ghRouting.points.length > 1) {
            ...
            ghRouting.doRequest(function (json) {
                if (json.message) {
                    ...
                    outHtml += "<br/>Energy used in KW: " + energy_calculation(path.distance, path.time);
...
function setupIsochrone(map, ghIsochrone) {
    map.setView([-34.928589,138.59994289999997], 14);
    var isochroneLayer;
